...
function findPendingJidForPoll(pollMsgId) {
      // Look up which JID this poll was sent to
...
async function handlePollVoteByHash(voterJid, selectedHashes, pollUpdate, sock) {
      try {
          ...
          if (optionName) {
                        // Use the stored JID (the phone number we sent the poll to)
...
async function handlePollVote(jid, optionName, sock) {
    ...
    const p = pending.get(jid);
      if (!p) {
                ...
                // Try all pending entries to find a match
...
module.exports = {
      ...
      findPendingJidForPoll: findPendingJidForPoll
};
