...
app.use(express.json({
  verify: (req, _res, buf) => {
    req.rawBody = buf;  // store raw Buffer for HMAC check
  }
}));
...
function verifyWebhook(req, res, next) {
  ...
  const hash = crypto
    ...
    .update(req.rawBody)  // use raw bytes, NOT JSON.stringify
    .digest('base64');
...
app.post('/webhooks/orders/create', verifyWebhook, async (req, res) => {
  ...
  if (process.env.ENABLE_ORDER_CONFIRMATION === 'true') {
    try {
      ...
      registerPendingOrder(phone, order);
    } catch (e) {
...
app.post('/webhooks/checkouts/create', verifyWebhook, async (req, res) => {
  ...
  setTimeout(async () => {
    ...
    try {
      ...
      // If we can't check, send the message anyway
    }
