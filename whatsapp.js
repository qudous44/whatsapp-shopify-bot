...
// Wait for WhatsApp to connect (up to maxWaitMs). Retries instead of failing.
async function waitForConnection(maxWaitMs) {
        if (!maxWaitMs) maxWaitMs = 120000; // 2 minutes
  if (status === 'connected') return;
        console.log('[Queue] WhatsApp not connected, waiting up to ' + Math.round(maxWaitMs/1000) + 's for reconnect...');
        const start = Date.now();
        while (status !== 'connected') {
                  if (Date.now() - start > maxWaitMs) {
                              throw new Error('WhatsApp not connected after waiting ' + Math.round(maxWaitMs/1000) + 's');
                  }
                  await new Promise(resolve => setTimeout(resolve, 3000));
        }
        console.log('[Queue] WhatsApp reconnected, proceeding with send.');
}
...
async function getWASocket() {
  ...
  sock.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
            ...
            if (connection === 'open') {
                        ...
                        // Resume queue processing after reconnect
              if (messageQueue.length > 0) {
